...
class Q_UML_EXPORT QUmlRemoveVariableValueActionObject : public QModelingObject
{
    ...
    Q_PROPERTY(QObject * variable READ variable WRITE setVariable NOTIFY variableChanged)
...
Q_SIGNALS:
    ...
    // Signals for owned attributes [VariableAction]
    void variableChanged(QObject *variable);

    // Signals for owned attributes [WriteVariableAction]
    void valueChanged(QObject *value);

    // Signals for owned attributes [RemoveVariableValueAction]
