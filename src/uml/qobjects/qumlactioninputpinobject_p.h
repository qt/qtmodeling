...
class Q_UML_EXPORT QUmlActionInputPinObject : public QModelingObject
{
    ...
    // Properties [Pin]
    Q_PROPERTY(bool isControl READ isControl WRITE setControl NOTIFY isControlChanged RESET unsetControl)

    // Properties [ActionInputPin]
    Q_PROPERTY(QObject * fromAction READ fromAction WRITE setFromAction NOTIFY fromActionChanged)
...
Q_SIGNALS:
    ...
    // Signals for owned attributes [Pin]
    void isControlChanged(bool isControl);

    // Signals for owned attributes [ActionInputPin]
    void fromActionChanged(QObject *fromAction);
