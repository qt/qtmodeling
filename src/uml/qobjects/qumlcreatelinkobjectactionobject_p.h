...
class Q_UML_EXPORT QUmlCreateLinkObjectActionObject : public QModelingObject
{
    ...
    Q_PROPERTY(QSet<QObject *> endData READ endData NOTIFY endDataChanged)

    // Properties [CreateLinkObjectAction]
    Q_PROPERTY(QObject * result READ result WRITE setResult NOTIFY resultChanged)

public:
    Q_INVOKABLE explicit QUmlCreateLinkObjectActionObject(QUmlCreateLinkObjectAction *modelingElement);
...
Q_SIGNALS:
    ...
    // Signals for owned attributes [LinkAction]
    void inputValuesChanged(QSet<QObject *> inputValues);

    // Signals for owned attributes [CreateLinkAction]
    void endDataChanged(QSet<QObject *> endData);

    // Signals for owned attributes [CreateLinkObjectAction]
