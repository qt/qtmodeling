...
class Q_UML_EXPORT QUmlInterfaceRealizationObject : public QModelingObject
{
    ...
    Q_PROPERTY(QSet<QObject *> clients READ clients NOTIFY clientsChanged)
    Q_PROPERTY(QSet<QObject *> suppliers READ suppliers NOTIFY suppliersChanged)

    // Properties [Abstraction]
    Q_PROPERTY(QObject * mapping READ mapping WRITE setMapping NOTIFY mappingChanged)

    // Properties [InterfaceRealization]
    Q_PROPERTY(QObject * contract READ contract WRITE setContract NOTIFY contractChanged)
    Q_PROPERTY(QObject * implementingClassifier READ implementingClassifier WRITE setImplementingClassifier NOTIFY implementingClassifierChanged)

public:
    Q_INVOKABLE explicit QUmlInterfaceRealizationObject(QUmlInterfaceRealization *modelingElement);
...
Q_SIGNALS:
    ...
    // Signals for owned attributes [Dependency]
    void clientsChanged(QSet<QObject *> clients);
    void suppliersChanged(QSet<QObject *> suppliers);

    // Signals for owned attributes [Abstraction]
    void mappingChanged(QObject *mapping);

    // Signals for owned attributes [InterfaceRealization]
    void contractChanged(QObject *contract);
    void implementingClassifierChanged(QObject *implementingClassifier);
