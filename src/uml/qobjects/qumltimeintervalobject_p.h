...
class Q_UML_EXPORT QUmlTimeIntervalObject : public QModelingObject
{
    ...
    Q_PROPERTY(QObject * max READ max WRITE setMax NOTIFY maxChanged)
    Q_PROPERTY(QObject * min READ min WRITE setMin NOTIFY minChanged)

public:
    Q_INVOKABLE explicit QUmlTimeIntervalObject(QUmlTimeInterval *modelingElement);
...
Q_SIGNALS:
    ...
    // Signals for owned attributes [Interval]

    // Signals for owned attributes [TimeInterval]
    void maxChanged(QObject *max);
    void minChanged(QObject *min);
